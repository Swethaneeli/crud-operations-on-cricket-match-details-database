...
//2) Get a specific player based on playerId
...
//3) Update details of a specific player
...
// 4) get match details of a specific match
...
//6) get all the players in a specific match

app.get("/matches/:matchId/players", async (request, response) => {
  const { matchId } = request.params;
  const getPlayerDetailsOfMatchQuery = `
    SELECT 
        *
    FROM 
        player_details
    ...
    WHERE 
        match_id = ${matchId};`;
  const players = await db.all(getPlayerDetailsOfMatchQuery);
  response.send(
